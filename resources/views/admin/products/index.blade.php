            ...
            @forelse($products as $product)
                <tr>
                    <td>
                        ...
                        @else
                            <div style="width:46px;height:46px;background:#09150f;border-radius:4px;border:1px solid var(--border-panel);display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:0.7rem">Không ảnh</div>
                        @endif
                    </td>
                    <td>
                    ...
                    <td>
                        ...
                        <div class="muted" style="font-size:0.82rem;margin-top:2px">
                            {{ number_format($product->variants->min('price') ?? 0, 0, ',', '.') }}–{{ number_format($product->variants->max('price') ?? 0, 0, ',', '.') }} ₫ ·
                            <span style="color:var(--lime)">{{ $product->variants->sum('stock') }} tồn</span>
                        </div>
                    </td>
                    <td>
