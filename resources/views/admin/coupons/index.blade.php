        ...
        <table class="table">
            ...
            <tbody>
                @forelse($coupons as $coupon)
                    <tr>
                        <td>
                            <span class="status mono" style="background:#132a1e;color:var(--lime);font-size:0.95rem;font-weight:700;letter-spacing:1px">
                                {{ $coupon->code }}
                            </span>
                        </td>
                        <td>
                            <b class="mono" style="color:var(--lime)">
                                {{ $coupon->type === 'percent' ? $coupon->value.'%' : number_format($coupon->value, 0, ',', '.').' ₫' }}
                            </b>
                        </td>
                        <td class="mono">{{ number_format($coupon->minimum_order_value, 0, ',', '.') }} ₫</td>
                        <td>
