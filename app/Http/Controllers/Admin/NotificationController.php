...
{
    public function index(Request $request): JsonResponse { return response()->json(['unread_count' => AdminNotification::where('user_id', $request->user()->id)->whereNull('read_at')->count(), 'data' => AdminNotification::where('user_id', $request->user()->id)->latest()->paginate($request->integer('per_page', 30))]); }
    public function syncLowStock(AdminNotificationService $service): JsonResponse { $service->syncImportantSizeLowStock(); return response()->json(['synced' => true]); }
