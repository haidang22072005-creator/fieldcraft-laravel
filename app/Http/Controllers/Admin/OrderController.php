...
class OrderController extends Controller
{
    public function dashboard(Request $request, AdminIntelligenceService $intelligence): View
    {
        ...
        $intelligenceData = $intelligence->dashboard();
        ...
        $chart = $range === 12 ? $revenueIntelligence['12m'] : $revenueIntelligence[(string) $range];
        $chartLabels = collect($chart['periods'])->pluck('period')->map(fn (string $period) => $range === 12 ? now()->createFromFormat('Y-m', $period)->format('m/Y') : now()->createFromFormat('Y-m-d', $period)->format('d/m'))->all();
        $chartValues = collect($chart['periods'])->pluck('completed_revenue')->map(fn ($value) => (int) $value)->all();
        ...
        return view('admin.dashboard', [
            ...
            'range' => $range, 'chartLabels' => $chartLabels, 'chartValues' => $chartValues,
            'revenue' => $intelligenceData['completed_revenue'], 'monthRevenue' => $intelligenceData['revenue_this_month'],
            'ordersCount' => $intelligenceData['total_orders'], 'pendingOrderCount' => $intelligenceData['pending_orders'], 'completedOrderCount' => $intelligenceData['completed_orders'],
            'customerCount' => $intelligenceData['customer_count'], 'productsCount' => Product::where('is_active', true)->count(), 'pendingReviewCount' => $intelligenceData['pending_review_count'],
...
return back()->with('success', 'Đã cập nhật trạng thái đơn.');
    ...
    public function syncGhn(Order $order, GHNOrderService $ghnOrders): RedirectResponse
    {
        if (! $order->ghn_order_code) {
            throw ValidationException::withMessages(['order' => 'Đơn hàng chưa có mã vận đơn GHN.']);
        } $tracking = $ghnOrders->trackingForOrder($order);
    ...
    public function manualComplete(Order $order, Request $request): RedirectResponse
    {
        ...
        DB::transaction(function () use ($order, $request): void {
            ...
            if ($locked->status !== 'cancelled' && $locked->status !== 'completed') {
                $from = $locked->status;
                $locked->update(['status' => 'completed', 'shipping_status' => 'delivered', 'completed_at' => $locked->completed_at ?? now(), 'completed_by' => $locked->completed_by ?? $request->user()->id]);
    ...
    {
        if ($from !== $to) {
            $order->statusHistories()->create(['from_status' => $from, 'to_status' => $to, 'source' => $source, 'actor_id' => $actor]);
        }
    }
}
