...
{
    public function index(Request $request, BootPassportService $service): JsonResponse { abort_unless($request->user()->role === 'customer', 403); $service->generateForUser($request->user()->id); return response()->json(['data' => $request->user()->bootPassports()->with(['order', 'orderItem', 'variant'])->latest()->get()]); }
